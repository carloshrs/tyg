...
namespace ar.com.TiempoyGestion.FrontEnd.Intranet.Embargo
{
	/// <summary>
	...
	public partial class Informe : Page
	{
		...
		{
			GravamenesDal embargo = new GravamenesDal();
			bool cargar = embargo.Cargar(Id, "Embargos");
			if (cargar)
			{
		...
		{
			GravamenesDal oEmbargo = new GravamenesDal();
			bool cargar = oEmbargo.Cargar(int.Parse(idInforme.Value), "Embargos");
			// Usuario Logueado
			UsuarioAutenticado Usuario = (UsuarioAutenticado)Session["UsuarioAutenticado"];
			oEmbargo.IdCliente = Usuario.IdCliente;
			oEmbargo.IdUsuario = Usuario.IdUsuario;

			
			oEmbargo.IdInforme = int.Parse(idInforme.Value);
			oEmbargo.Folio = txtFolio.Text;
			oEmbargo.Tomo = txtTomo.Text;
			oEmbargo.Ano = txtAno.Text;
            oEmbargo.Observaciones = txtObservaciones.Text.ToUpper();

			if(int.Parse(idReferencia.Value) == 0)
				oEmbargo.Crear("Embargos");
			else
				oEmbargo.Modificar(int.Parse(idInforme.Value), "Embargos");
