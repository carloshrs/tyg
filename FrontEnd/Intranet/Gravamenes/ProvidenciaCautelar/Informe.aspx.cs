...
namespace ar.com.TiempoyGestion.FrontEnd.Intranet.Gravamenes.ProvidenciaCautelar
{
	...
	public partial class Informe : Page
	{
		...
		{
            GravamenesDal providenciacautelar = new GravamenesDal();
            bool cargar = providenciacautelar.Cargar(Id, "providenciacautelar");
			if (cargar)
			{
				idReferencia.Value = (1).ToString();
                CargarForm(providenciacautelar);
			}
			else
			{
		...
		private void ActualizarInforme()
		{
            ...
            bool cargar = oProvidenciacautelar.Cargar(int.Parse(idInforme.Value), "providenciacautelar");
			// Usuario Logueado
			UsuarioAutenticado Usuario = (UsuarioAutenticado)Session["UsuarioAutenticado"];
            oProvidenciacautelar.IdCliente = Usuario.IdCliente;
            oProvidenciacautelar.IdUsuario = Usuario.IdUsuario;


            oProvidenciacautelar.IdInforme = int.Parse(idInforme.Value);
            oProvidenciacautelar.Folio = txtFolio.Text;
            oProvidenciacautelar.Tomo = txtTomo.Text;
            oProvidenciacautelar.Ano = txtAno.Text;
            oProvidenciacautelar.Observaciones = txtObservaciones.Text.ToUpper();

			if(int.Parse(idReferencia.Value) == 0)
                oProvidenciacautelar.Crear("providenciacautelar");
			else
                oProvidenciacautelar.Modificar(int.Parse(idInforme.Value), "providenciacautelar");
