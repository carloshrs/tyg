...
namespace ar.com.TiempoyGestion.FrontEnd.Intranet.BienFamilia
{
	/// <summary>
	...
	public partial class Informe : Page
	{
		...
		{
			GravamenesDal oBienFamilia = new GravamenesDal();
			bool cargar = oBienFamilia.Cargar(int.Parse(idInforme.Value), "BienFamilia");
			// Usuario Logueado
			UsuarioAutenticado Usuario = (UsuarioAutenticado)Session["UsuarioAutenticado"];
			oBienFamilia.IdCliente = Usuario.IdCliente;
			oBienFamilia.IdUsuario = Usuario.IdUsuario;
			
			oBienFamilia.IdInforme = int.Parse(idInforme.Value);
			oBienFamilia.Folio = txtFolio.Text;
			oBienFamilia.Tomo = txtTomo.Text;
			oBienFamilia.Ano = txtAno.Text;
			oBienFamilia.Observaciones = txtObservaciones.Text.ToUpper();

			if(int.Parse(idReferencia.Value) == 0)
				oBienFamilia.Crear("BienFamilia");
			else
				oBienFamilia.Modificar(int.Parse(idInforme.Value), "BienFamilia");
