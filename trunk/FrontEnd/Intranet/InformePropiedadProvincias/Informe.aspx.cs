...
namespace ar.com.TiempoyGestion.FrontEnd.Intranet.InformePropiedadOtrasProvincias
{
	...
	public partial class Informe : Page
	{
        ...
        private void ControlarSolicitudesMatricula(int id, int tipoInforme, int tipoPropiedad, int provincia, string leg, string folio, string tomo, string ano)
		{
            ...
            if (txtInformesAnteriores.Text == "")
            {
                ...
                {
                    case 1: solicitudes = ControlMatriculasDal.ControlarMatricula(idInforme, tipoInforme, tipoPropiedad, provincia, legajo);
                            break;
                    case 2: solicitudes = ControlMatriculasDal.ControlarMatricula(idInforme, tipoInforme, tipoPropiedad, provincia, legajo, tomo, folio, ano);
                            break;
                    case 3: solicitudes = ControlMatriculasDal.ControlarMatricula(idInforme, tipoInforme, tipoPropiedad, provincia, legajo, tomo, folio, ano);
                            break;
                }
		...
		private void CargarEncabezado(EncabezadoApp oEncabezado)
		{
			...
			CargarComboProvincias(cmbProvincia, oEncabezado.ProvinciaOtra);
			CargarComboLocalidades(cmbProvincia, cmbLocalidad, oEncabezado.LocalidadOtra.ToString());
		...
		protected void AceptarFinalizar_Click(object sender, System.EventArgs e)
		{
			string strScript;
			strScript = "<script languaje=\"Javascript\">";
            strScript += "window.showModalDialog('/BandejaEntrada/PopUpCambioEstado.aspx?idTipo=11&idInforme=" + idInformePropiedad.Value + "&Finalizar=1','','dialogWidth:400px;dialogHeight:250px');";
			strScript += "document.location.href = '/BandejaEntrada/Principal.aspx?idTipo=11'";
			strScript += "</script>";
		...
		protected void Cancelar_Click(object sender, EventArgs e)
		{
			...
			Response.Redirect("/BandejaEntrada/Principal.aspx?idTipo=11");
		}
...
protected void  btnImprimir_Click(object sender, EventArgs e)
{
    ...
    string strScript;
	strScript = "<script languaje=\"Javascript\">";
	strScript += "window.open('/InformePropiedadProvincias/VerInforme.aspx?id=" + idInformePropiedad.Value + "&IdTipo=11','','tools=no,width=720,menus=no,scrollbars=yes');";
...
protected void Rechazar_Click(object sender, EventArgs e)
{
    string strScript;
    strScript = "<script languaje=\"Javascript\">";
    strScript += "window.showModalDialog('/BandejaEntrada/PopUpCambioEstado.aspx?idTipo=11&idInforme=" + idInformePropiedad.Value + "&Rechazar=1','','dialogWidth:400px;dialogHeight:250px');";
    strScript += "document.location.href = '/BandejaEntrada/Principal.aspx?idTipo=11'";
    strScript += "</script>";
