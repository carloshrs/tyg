...
namespace ar.com.TiempoyGestion.FrontEnd.Intranet.Mensajeria
{
    ...
    public partial class pendientes : Page
    {
        ...
        private void ListaBandeja()
        {
            ...
            dgMensajeriaPendientes.DataSource = bandeja.ListaEncabezados(IdTipo, pFiltro, -1, idUser, Estados, 1, "", "", 100, false);
            dgMensajeriaPendientes.DataBind();
            if (dgMensajeriaPendientes.Items.Count == 0)
            {
                dgMensajeriaPendientes.Visible = false;
            ...
            if (dgMensajeriaPendientes.Items.Count == 0)
            {
                lblMensaje.Text = "No hay mensajerias para procesar";
        ...
        protected void dgMensajeriaPendientes_PreRender(object sender, EventArgs e)
        {
            string strDescripcion = "";
            foreach (DataGridItem myItem in dgMensajeriaPendientes.Items)
            {
        ...
        protected void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            if (dgMensajeriaPendientes.Items.Count > 0)
                intGrupo = Encabezado.crearGrupoCambioEstado(Usuario.IdUsuario, 22);

            foreach (DataGridItem dgSUrg in dgMensajeriaPendientes.Items)
			{
        ...
        {
            Response.Redirect("historialimpresiones.aspx?idTipo=22");
        }
}
}